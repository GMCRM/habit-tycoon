-- Notify the achiever themselves, not just their friends.
--
-- Both achievement systems (general_achievements via
-- notify_friends_of_general_achievement, and per-habit milestones via
-- notify_friends_of_milestone — see 20260722235413_professional_achievement_notifications.sql
-- for the current bodies) only ever wrote rows into social_pokes for the
-- achiever's accepted friends. A user with zero friends got no in-app
-- notification at all when they earned something, and even a user with
-- friends never saw their own achievement show up in their own
-- notifications tab.
--
-- This re-declares both functions (same signatures, so no DROP needed) to
-- additionally insert one self-addressed social_pokes row (from_user_id =
-- to_user_id = the achiever) alongside the existing friend rows. Reuses the
-- exact same `type` and `metadata` shape as the friend-facing rows
-- ('general_achievement' / 'friend_milestone') so the existing icon
-- rendering in social.page.html (keyed off notification.type and
-- notification.metadata, never from_user_id) picks it up with no client
-- changes. Only the message text differs ("You" instead of the user's own
-- name, which would read strangely).
--
-- Respects the same p_notify gating as the friend notifications: both
-- functions are already only called (from check_general_achievements /
-- the milestone check path) when the caller wants notifications sent, so
-- the one-time silent backfills in prior migrations remain silent for the
-- achiever too, not just their friends.
...
BEGIN
    ...
    v_self_message := 'You just earned the ' || p_emoji || ' ' || p_label ||
        ' achievement' || COALESCE(' — ' || NULLIF(p_description, ''), '') || '.';
        ...
        THEN
            ...
            IF v_inserted_id IS NOT NULL THEN
                ...
                v_self_message := 'You just earned the ' || v_milestone.emoji || ' ' ||
                    v_milestone.label || ' milestone on your ' || v_business_type_icon || ' ' ||
                    v_business_type_name || ' — ' || v_milestone.description || '.';
