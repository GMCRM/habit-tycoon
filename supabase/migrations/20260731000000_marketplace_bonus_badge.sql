-- Persist the streak-bonus % a Marketplace purchase baked into a business's
-- earnings, so the Home Screen habit card can show the same "+X%" badge the
-- Marketplace listing showed (getStreakBonusPercent() in social.page.ts).
-- Previously this bonus was folded into earnings_per_completion by
-- resolve_marketplace_purchase() and then had no separate visible trace.

ALTER TABLE habit_businesses
    ADD COLUMN IF NOT EXISTS marketplace_bonus_percent SMALLINT NULL;

-- ─── resolve_marketplace_purchase: also stash the bonus % onto the resulting business ───
