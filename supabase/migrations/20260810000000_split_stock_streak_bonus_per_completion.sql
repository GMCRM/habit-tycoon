-- Fix: for multi-completion-per-day habits (goal_value > 1), the stock-boost
-- and streak-bonus portions of a day's earnings were only ever paid on the
-- completion that happened to finish the day's goal — every earlier
-- completion in the same day earned just the plain base share
-- (earnings_per_completion, i.e. base_pay / goal_value) with zero bonus.
--
-- e.g. a habit worth $25/day (base pay + stock boost + streak bonus) with
-- goal_value = 5 should pay $5 on every one of the 5 taps. Before this fix
-- it paid ~$4 (base_pay / 5, no bonus) on taps 1-4 and dumped the whole
-- day's stock boost + streak bonus onto tap 5 alone.
--
-- Fix: compute the stock boost and the projected streak-bonus multiplier on
-- every completion, not just the goal-completing one, so each tap earns an
-- equal share of the day's total (base + boost + bonus) / goal_value. The
-- streak *lookback* (whether the previous period's goal was met) only
-- depends on data from before today, so it's safe and idempotent to compute
-- on every tap — only the persisted habit_businesses.streak column and the
-- habit_completions.streak_count of non-final taps are left untouched,
-- exactly as before, since the streak itself only "officially" advances
-- once the day's goal is actually met.
...
BEGIN
    ...
    -- Stock boost and streak bonus are now computed (and paid) on every
