-- Re-apply 20260820200000_pay_dividends_on_every_completion.sql's fix on top
-- of complete_habit_business's newer live version.
--
-- 20260823000000_snapshot_business_type_on_completion.sql branched
-- complete_habit_business off the same pre-fix baseline
-- (20260810010000_fix_dividend_per_share_formula.sql) that
-- 20260820200000 also branched off, and landed on main after it. Its
-- CREATE OR REPLACE silently reverted 20260820200000's fix: dividends
-- (process_habit_completion_dividends) went back to firing only
-- `IF v_is_goal_completed`, so a habit with goal_value > 1 (multiple
-- check-ins per day) once again pays stockholders on only the tap that
-- completes the day's goal, and nothing on every earlier tap — even
-- though the owner is correctly paid an equal split on every tap.
--
-- Fix: same change as 20260820200000 — move the
-- PERFORM process_habit_completion_dividends call out of the
-- `IF v_is_goal_completed` block so it runs on every completion.
-- Streak-price updates and milestone notifications stay gated to goal
-- completion, same as before. Everything else in this function is
-- byte-for-byte identical to the live version
-- (20260823000000_snapshot_business_type_on_completion.sql), including
-- the business_type_name/business_icon snapshot that landed since
-- 20260820200000 was written.
