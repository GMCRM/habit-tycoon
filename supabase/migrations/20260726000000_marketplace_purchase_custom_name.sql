-- Fix: "Start a New Habit Instead" always named the new business after the
-- seller's original listing name, with no way for the buyer to rename it.
-- Add an optional p_business_name override, used only on the new-habit path
-- (the merge path intentionally keeps taking on the purchased business's name,
-- since it's replacing the target business's type/icon/earnings wholesale).
