...
CREATE OR REPLACE FUNCTION extract_date_utc(ts timestamptz) RETURNS date LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
...
WITH ranked_completions AS (
    SELECT id,
        ROW_NUMBER() OVER (
            PARTITION BY habit_business_id,
            user_id,
...
BEGIN GET DIAGNOSTICS deleted_count = ROW_COUNT;
RAISE NOTICE 'Removed % duplicate habit completions',
deleted_count;
END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_daily_completion ON habit_completions(
    habit_business_id,
    user_id,
    extract_date_utc(completed_at)
);
