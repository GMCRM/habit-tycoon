-- SAFER corrected portfolio function 
-- This version will work regardless of user_profiles structure
...
SELECT gen_random_uuid() as holding_id,
    ...
    -- Use business type name (e.g., "Lemonade Stand") instead of habit name
    COALESCE(bt.name, 'Unknown Business') as business_name,
    COALESCE(bt.icon, '🏢') as business_icon,
    -- Try to get user name, fallback to owner ID if user_profiles doesn't work
    COALESCE(
        CASE
            WHEN up.name IS NOT NULL THEN up.name
            WHEN up.display_name IS NOT NULL THEN up.display_name
            ELSE CONCAT(
                'User-',
                SUBSTRING(bs.business_owner_id::text, 1, 8)
            )
        END,
        'Unknown Owner'
    ) as owner_name,
...
FROM user_holdings uh
    ...
    JOIN business_types bt ON hb.business_type_id = bt.id -- Get business type names
    LEFT JOIN user_profiles up ON bs.business_owner_id = up.id -- Try to get owner names
