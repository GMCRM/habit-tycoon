...
BEGIN 
    -- Validation: Check if completion exists
    IF completion_uuid IS NULL THEN
        RAISE WARNING 'process_habit_completion_dividends: completion_uuid is NULL';
        RETURN;
    END IF;
    ...
    IF habit_business_uuid IS NULL THEN
        RAISE WARNING 'process_habit_completion_dividends: No habit completion found for UUID %', completion_uuid;
        RETURN;
    END IF;
    
    -- Validation: Check earnings is valid
    IF base_earnings IS NULL OR base_earnings <= 0 THEN
        RAISE WARNING 'process_habit_completion_dividends: Invalid base_earnings % for completion %', base_earnings, completion_uuid;
        RETURN;
    END IF;
    ...
    IF total_shares_issued IS NULL OR total_shares_issued <= 0 THEN
        RAISE WARNING 'process_habit_completion_dividends: Invalid total_shares_issued % for stock %', total_shares_issued, stock_uuid;
        RETURN;
    END IF;
    
    IF shares_owned_by_owner IS NULL OR shares_owned_by_owner < 0 THEN
        RAISE WARNING 'process_habit_completion_dividends: Invalid shares_owned_by_owner % for stock %', shares_owned_by_owner, stock_uuid;
        RETURN;
    END IF;
    
    IF shares_owned_by_owner > total_shares_issued THEN
        RAISE WARNING 'process_habit_completion_dividends: Owner shares % exceeds total shares % for stock %', shares_owned_by_owner, total_shares_issued, stock_uuid;
        RETURN;
    END IF;
    ...
    IF shares_owned_by_others <= 0 THEN
        RETURN;
    END IF;
    ...
    INTO total_external_shares
    FROM stock_holdings
    WHERE stock_id = stock_uuid;
    
    -- Validation: Make sure we have shares to distribute to
    IF total_external_shares IS NULL OR total_external_shares <= 0 THEN
        RAISE WARNING 'process_habit_completion_dividends: No external shares found for stock % despite shares_owned_by_others=%', stock_uuid, shares_owned_by_others;
        RETURN;
    END IF;
    ...
    FOR stockholder IN
        ...
        AND shares_owned > 0
    LOOP 
        -- Validation: Skip invalid holdings
        IF stockholder.shares_owned IS NULL OR stockholder.shares_owned <= 0 THEN
            CONTINUE;
        END IF;
        ...
        IF stockholder_dividend <= 0 THEN
            CONTINUE;
        END IF;
        ...
        SET 
            cash = cash + stockholder_dividend,
            updated_at = NOW()
        ...
        SET 
            total_dividends_earned = total_dividends_earned + stockholder_dividend,
            updated_at = NOW()
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE WARNING 'process_habit_completion_dividends: Unexpected error for completion %: %', completion_uuid, SQLERRM;
...
COMMENT ON FUNCTION process_habit_completion_dividends(UUID) IS 
'Processes dividend payments to stockholders when a habit business owner completes a habit. 
